...
def get_movable_method(table, r, c):
    ...
    if (c > 2 and r > -2 and r < R-1):
        if r==-1:
            ...
            if (table[r][c-2] == 0 and table[r-1][c-1] == 0 and table[r+1][c-1] == 0 and table[r+1][c-2] == 0 and table[r+2][c-1] == 0):
                return 2
...
for i in range(4):
    if i == directs[1]:
        table[fairy[0]+direction[i][0]][fairy[1]+direction[i][1]] = 2
    else:
    ...
    while True:
        if (method == -1):
            if (r<2):
                ...
                for j in range(4):
                    if j == directs[i]:
                        table[r+direction[j][0]][c+direction[j][1]] = 2
                    else:
                        table[r+direction[j][0]][c+direction[j][1]] = 1
